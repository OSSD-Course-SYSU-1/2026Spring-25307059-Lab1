...
import type { AppMigrationState } from '../common/utils/DistributedStateManager';
...
export default class DrawModel {
    ...
    drawFlower() {
        ...
        // Get custom colors
        const colorConfig = this.configManager.getColorConfig();
        const flowerOutColor = colorConfig.flowerOutColor;
        const flowerInnerColor = colorConfig.flowerInnerColor;
        for (let i = 0; i < CommonConstants.COUNT; i++) {
            this.canvasContext?.save();
            this.canvasContext?.rotate(beginAngle * Math.PI / CommonConstants.HALF_CIRCLE);
            this.fillArc(new FillArcData(0, -pointY, radius, 0, Math.PI * CommonConstants.TWO), flowerOutColor);
            this.fillArc(new FillArcData(0, -pointY, innerRadius, 0, Math.PI * CommonConstants.TWO), flowerInnerColor);
    ...
    drawOutCircle() {
        ...
        const colorConfig = this.configManager.getColorConfig();
        // Draw outer disc.
        this.fillArc(new FillArcData(0, 0, this.screenWidth * CommonConstants.OUT_CIRCLE_RATIOS, 0, Math.PI * CommonConstants.TWO), colorConfig.outCircleColor);
    ...
    drawInnerCircle() {
        // Get custom colors
        const colorConfig = this.configManager.getColorConfig();
        this.fillArc(new FillArcData(0, 0, this.screenWidth * CommonConstants.INNER_CIRCLE_RATIOS, 0, Math.PI * CommonConstants.TWO), colorConfig.innerCircleColor);
     ...
     */
    drawInnerArc() {
        // Get custom arc colors
        const colors = this.configManager.getArcColors();
    ...
    drawArcText() {
        ...
        if (this.canvasContext !== undefined) {
            ...
            this.canvasContext.fillStyle = colorConfig.textColor;
            this.canvasContext.font = StyleConstants.ARC_TEXT_SIZE + CommonConstants.CANVAS_FONT;
        }
        // Get custom display names
        const displayNames = this.configManager.getDisplayNames();
        let arcTextStartAngle = CommonConstants.ARC_START_ANGLE;
        let arcTextEndAngle = CommonConstants.ARC_END_ANGLE;
        for (let i = 0; i < CommonConstants.COUNT; i++) {
            this.drawCircularText(displayNames[i], (this.startAngle + arcTextStartAngle) * Math.PI / CommonConstants.HALF_CIRCLE, (this.startAngle + arcTextEndAngle) * Math.PI / CommonConstants.HALF_CIRCLE);
            this.startAngle += this.avgAngle;
        }
    }
    /**
     ...
     */
    restoreState(state: AppMigrationState): void {
        ...
        Logger.info('[DrawModel] State restored from migration');
    }
    /**
